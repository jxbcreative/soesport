...
const Products: React.FC<PropProduct> = ({
  openFilter,
  selectSorting,
  sorting,
  selectMerek,
  ...
  useEffect(() => {
    ...
    // select product
    selectMerek.length === 0
      ? product
      : (filterProduct = filterProduct.filter((items) =>
          selectMerek.includes(items.merek)
        ));
      ...
      {isMobile ? (
        ...
        <div className="px-3 mt-10">
          <div className="grid grid-cols-2 gap-5">
            {filteredProduct
              .slice(0, showMore ? filteredProduct.length : 8)
              ?.map((items: any) => (
                <div key={items.id}>
          ...
          <div className="text-center mt-10 text-white">
            {showMore ? (
              <button
                ...
                className="bg-blue-500 py-2.5 px-6 rounded-md font-medium"
              >
                Hide more
              </button>
            ) : (
